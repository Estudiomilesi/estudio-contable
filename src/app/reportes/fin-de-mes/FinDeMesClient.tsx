            ...
            {alertasParticipacion.map((a, i) => (
              <li key={i} className="text-sm bg-white p-2 rounded border border-red-200 shadow-sm flex justify-between">
                ...
                <div className="font-bold text-red-700">
                  ${a.amount.toLocaleString('es-AR', {minimumFractionDigits:2})}
                </div>
              </li>
            ))}
          </ul>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-semibold">${resultadoF.toLocaleString('es-AR', {minimumFractionDigits: 2})}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">50% Resultado Estudio FJ:</span>
              <span className="font-semibold">${(resultadoFJ * 0.5).toLocaleString('es-AR', {minimumFractionDigits: 2})}</span>
            </div>
            <div className="flex justify-between border-t pt-2 font-bold text-indigo-900">
              <span>Total a Distribuir:</span>
              <span>${fedeShare.toLocaleString('es-AR', {minimumFractionDigits: 2})}</span>
            </div>
            <div className="flex justify-between text-red-600 pt-2">
              <span>(-) Retiros Fede:</span>
              <span>-${retirosFede.toLocaleString('es-AR', {minimumFractionDigits: 2})}</span>
            </div>
          </div>
          <div className={`mt-4 p-3 rounded-lg text-center ${fedeBalance > 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            ...
            <span className="text-2xl font-black">${Math.abs(fedeBalance).toLocaleString('es-AR', {minimumFractionDigits: 2})}</span>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-600">50% Resultado Estudio FJ:</span>
              <span className="font-semibold">${juanmaShare.toLocaleString('es-AR', {minimumFractionDigits: 2})}</span>
            </div>
            <div className="flex justify-between border-t pt-2 font-bold text-indigo-900 mt-[26px]">
              <span>Total a Distribuir:</span>
              <span>${juanmaShare.toLocaleString('es-AR', {minimumFractionDigits: 2})}</span>
            </div>
            <div className="flex justify-between text-red-600 pt-2">
              <span>(-) Retiros Juanma:</span>
              <span>-${retirosJuanma.toLocaleString('es-AR', {minimumFractionDigits: 2})}</span>
            </div>
          </div>
          <div className={`mt-4 p-3 rounded-lg text-center ${juanmaBalance > 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            ...
            <span className="text-2xl font-black">${Math.abs(juanmaBalance).toLocaleString('es-AR', {minimumFractionDigits: 2})}</span>
          </div>
        </div>
      </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-800 text-white">
            <tr>
              ...
              <th className="px-6 py-4 text-right text-sm font-semibold uppercase tracking-wider border-l border-gray-600 bg-green-900">Estudio F</th>
              <th className="px-6 py-4 text-right text-sm font-semibold uppercase tracking-wider border-l border-gray-600 bg-orange-900">Estudio FJ</th>
              <th className="px-6 py-4 text-right text-sm font-semibold uppercase tracking-wider border-l border-gray-600">Consolidado</th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-200">
            ...
            <tr className="bg-gray-50">
              ...
              <td className="px-6 py-3 text-right text-green-700 font-bold border-l border-gray-200">${ingresosF.toLocaleString('es-AR', {minimumFractionDigits:2})}</td>
              <td className="px-6 py-3 text-right text-orange-700 font-bold border-l border-gray-200">${ingresosFJ.toLocaleString('es-AR', {minimumFractionDigits:2})}</td>
              <td className="px-6 py-3 text-right text-gray-900 font-bold border-l border-gray-200">${ingresosConsolidado.toLocaleString('es-AR', {minimumFractionDigits:2})}</td>
            </tr>
            {/* Gastos */}
            <tr>
              <td className="px-6 py-3 font-bold text-red-800">Egresos Operativos</td>
              <td className="px-6 py-3 text-right text-red-600 border-l border-gray-200">-${gastosF.toLocaleString('es-AR', {minimumFractionDigits:2})}</td>
              <td className="px-6 py-3 text-right text-red-600 border-l border-gray-200">-${gastosFJ.toLocaleString('es-AR', {minimumFractionDigits:2})}</td>
              <td className="px-6 py-3 text-right text-red-600 font-bold border-l border-gray-200">-${gastosConsolidado.toLocaleString('es-AR', {minimumFractionDigits:2})}</td>
            </tr>
          </tbody>
          <tfoot className="bg-gray-100 font-black text-lg">
            <tr>
              <td className="px-6 py-4 text-gray-900">Resultado Neto</td>
              <td className="px-6 py-4 text-right border-l border-gray-300 text-green-900">${resultadoF.toLocaleString('es-AR', {minimumFractionDigits:2})}</td>
              <td className="px-6 py-4 text-right border-l border-gray-300 text-orange-900">${resultadoFJ.toLocaleString('es-AR', {minimumFractionDigits:2})}</td>
              <td className="px-6 py-4 text-right border-l border-gray-300 text-indigo-900">${resultadoConsolidado.toLocaleString('es-AR', {minimumFractionDigits:2})}</td>
            </tr>
          </tfoot>
        </table>
      </div>
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-2 text-right font-medium text-gray-500">Asignación</th>
                <th className="px-4 py-2 text-right font-medium text-gray-500">Importe Total</th>
              </tr>
            </thead>
